...
import { queryClient, apiRequest, setApiBase } from "../lib/queryClient";
...
import { WorkObjectsListDialog } from "./WorkObjectsListDialog";
import { ScheduleCallDialog, MeetingsListDialog } from "./ScheduleCallDialog";
...
export interface ChatAppProps {
  /** Currently signed-in user. Host app supplies this (main app via its
   *  AuthProvider/useAuth, widget via its own cookie-session ChatApiClient). */
  user: ApiUser | null | undefined;
  /**
   * Base URL of the Chat API, e.g. "https://chat.bulldogops.com". Optional —
   * omit for same-origin use (the main app's existing behavior, unchanged).
   * When provided, every fetch/EventSource/upload call this render tree
   * makes is redirected there (used by the widget when mounted cross-origin
   * on Contracts/Ops).
   */
  apiBaseUrl?: string;
  /** Deep-link entrypoint: scope ChatApp directly to a channel on mount
   *  (used by the widget's `bulldog:widget:openJob` handling). */
  initialChannelId?: number | null;
}

// The full chat experience (project rail + channel sidebar + channel view +
// right rail) extracted from client/src/pages/Home.tsx so both the main app
// and the widget can mount exactly the same UI. The main app's Home.tsx is
// now a thin wrapper: `<ChatApp user={user} />` (see client/src/pages/Home.tsx).
export function ChatApp({ user, apiBaseUrl, initialChannelId }: ChatAppProps) {
  useEffect(() => {
    if (apiBaseUrl) setApiBase(apiBaseUrl);
  }, [apiBaseUrl]);

  // Widget deep-link: when opened scoped to a specific job/channel, jump
  // straight there once projects/channels are loaded (handled below, near
  // the other deep-link effects).
  const [widgetChannelPending, setWidgetChannelPending] = useState(initialChannelId ?? null);
  ...
  // Widget entrypoint: `initialChannelId` (from the `bulldog:widget:openJob`
  // handler resolving a job's first channel) resolves exactly like a URL
  // deep-link, minus any URL manipulation (the widget isn't on a page that
  // owns /#/channels/ routing). Reuses the same channel-fetch shape as the
  // deep-link effect above so DM vs project-channel routing stays identical.
  useEffect(() => {
    if (widgetChannelPending == null) return;
    const channelId = widgetChannelPending;
    let cancelled = false;
    (async () => {
      try {
        const ch = await apiRequest<{ id: number; projectId: number | null; scope?: string | null }>(
          "GET",
